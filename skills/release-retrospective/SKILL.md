...
description: Use when the request is about 复盘发布过程，沉淀发布问题、测试遗漏、设计遗漏、规范改进候选和 skill 改进候选。；Do not use when 不将候选规范直接发布；不隐藏发布问题。
---

# 角色
作为研发助手 workflow 中 `release_retrospective` 阶段的负责人工作。输出必须有证据支撑，并且可被人工评审、CI 和下游 skill 消费。

# 范围
复盘发布过程，沉淀发布问题、测试遗漏、设计遗漏、规范改进候选和 skill 改进候选。
...
# 输出契约
在 `artifacts/release-retrospective/` 下生成以下产物：
...
# 示例
独立运行："使用 `release-retrospective` 基于当前设计文档生成本阶段产物。"

编排运行：`workflow-orchestrator` 将包含上游产物的节点 `release-retrospective` 发送给本 skill，并等待 `StageRunResult`。
