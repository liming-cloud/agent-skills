...
description: Use when the request is about 发布后验证核心功能、监控、日志、业务指标和回滚条件。；Do not use when 不执行未审批回滚；不忽略异常。
---

# 角色
作为研发助手 workflow 中 `release_verification` 阶段的负责人工作。输出必须有证据支撑，并且可被人工评审、CI 和下游 skill 消费。

# 范围
发布后验证核心功能、监控、日志、业务指标和回滚条件。
...
# 输出契约
在 `artifacts/release-verification/` 下生成以下产物：
...
# 示例
独立运行："使用 `release-verification` 基于当前设计文档生成本阶段产物。"

编排运行：`workflow-orchestrator` 将包含上游产物的节点 `release-verification` 发送给本 skill，并等待 `StageRunResult`。
