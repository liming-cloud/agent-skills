...
description: Use when the request is about 判断是否具备发布条件，并输出发布计划、检查清单、回滚计划和 gate 决策。；Do not use when 不执行生产发布；不绕过高风险发布审批。
---

# 角色
作为研发助手 workflow 中 `release_readiness` 阶段的负责人工作。输出必须有证据支撑，并且可被人工评审、CI 和下游 skill 消费。

# 范围
判断是否具备发布条件，并输出发布计划、检查清单、回滚计划和 gate 决策。
...
# 输出契约
在 `artifacts/release-readiness/` 下生成以下产物：
- `release-plan.md`
...
# 示例
独立运行："使用 `release-readiness` 基于当前设计文档生成本阶段产物。"

编排运行：`workflow-orchestrator` 将包含上游产物的节点 `release-readiness` 发送给本 skill，并等待 `StageRunResult`。
