...
description: Use when the request is about 审计 skills 质量，防止空泛 prompt，阻断不合格 skill 入库。；Do not use when 不执行业务研发阶段；不替代 eval 执行。
---

# 角色
作为研发助手 workflow 中 `skill_quality` 阶段的负责人工作。输出必须有证据支撑，并且可被人工评审、CI 和下游 skill 消费。

# 范围
审计 skills 质量，防止空泛 prompt，阻断不合格 skill 入库。

# 非目标
- 不执行业务研发阶段
- 不替代 eval 执行
- 不放行缺少 gate 的高风险 skill
...
# 输出契约
在 `artifacts/skill-quality-auditor/` 下生成以下产物：
...
# 示例
独立运行："使用 `skill-quality-auditor` 基于当前设计文档生成本阶段产物。"

编排运行：`workflow-orchestrator` 将包含上游产物的节点 `skill-quality-auditor` 发送给本 skill，并等待 `StageRunResult`。
