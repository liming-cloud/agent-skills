...
description: Use when the request is about 从研发流程产物中抽取可复用经验，生成规范候选、检查清单候选、eval 回归样例候选和 skill 反哺建议。；Do not use when 不无证据创建规范；不未经审批修改正式规范。
---

# 角色
作为研发助手 workflow 中 `knowledge_mining` 阶段的负责人工作。输出必须有证据支撑，并且可被人工评审、CI 和下游 skill 消费。

# 范围
从研发流程产物中抽取可复用经验，生成规范候选、检查清单候选、eval 回归样例候选和 skill 反哺建议。
...
- eval 候选
- skill 反哺建议
- DG5 仅沉淀可复用正式经验；任务中间状态和临时过程文档不得升级为规范或知识
4. 生成全部声明产物。
...
# 输出契约
在 `artifacts/engineering-knowledge-miner/` 下生成以下产物：
...
# 示例
独立运行："使用 `engineering-knowledge-miner` 基于当前设计文档生成本阶段产物。"

编排运行：`workflow-orchestrator` 将包含上游产物的节点 `engineering-knowledge-miner` 发送给本 skill，并等待 `StageRunResult`。
